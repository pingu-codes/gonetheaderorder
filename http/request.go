...
func (r *Request) AddCookie(c *Cookie) {
	fmt.Println(r.Header[HeaderOrderKey])
...
func (r *Request) write(w io.Writer, usingProxy bool, extraHeaders Header, waitForContinue func() bool) (err error) {
	...
	// Header lines
	if !contains(r.Header[HeaderOrderKey], "host") {
	...
	if !contains(r.Header[HeaderOrderKey], "user-agent") {
	...
	fmt.Println("exclude headers")
	if r.Header["Content-Length"] != nil {
		r.Header["Content-Length"] = []string{strconv.FormatInt(tw.ContentLength, 10)}
	}
	...
	if extraHeaders != nil {
		fmt.Println("testting", extraHeaders)
