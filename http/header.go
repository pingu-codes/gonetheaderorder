...
func (h Header) writeSubset(w io.Writer, exclude map[string]bool, trace *httptrace.ClientTrace) error {
	...
	fmt.Println("exlcuding")
	fmt.Println(exclude)
	// _, err := io.WriteString(w, "testing: test")
	// if err != nil {
	// 	fmt.Println(err)
	// }
	...
	if headerOrder, ok := h[HeaderOrderKey]; ok {
		...
		fmt.Println("HEADER ORDER", headerOrder)
		fmt.Println("ORDER", order, exclude)

		kvs, sorter = h.sortedKeyValuesBy(order, exclude)
		fmt.Println("ORDER2", kvs)
	...
	for _, kv := range kvs {
		for _, v := range kv.values {
			...
			if kv.key == "Content-Length" {
				fmt.Println(kv.key, "aaaaa")
			}
			for _, s := range []string{kv.key, ": ", v, "\r\n"} {

				// fmt.Println("ws writing", s)
